...
*
* @param <NTOCH> the number of prediction points.
* @param <BX> the initial vector of orbital elements.
* @param <BIX> the prediction mooments.
* @param <TKNK> the prediction mooments.
*
* @return the array of predicted orbital elements.
*/
//------------------------------------------------------------------------------
...
//********************************************8.05.97********************
...
#endif
                            ;
                        if((XAXA=FORCE(T+CH2,Q1,PQ2)))  break;
                        ;
                        for (J=0; J<=5; J++)
                            Q1[J]=ER[J]+CH2*PQ2[J]
#if PECHAT1
...
#endif
                            ;
                        if((XAXA=FORCE(T+CH2,Q1,PQ3)))  break;
                        ;
                        for (J=0; J<=5; J++)
                            Q1[J]=ER[J]+CH*PQ3[J]
#if PECHAT1
...
#endif
                            ;
                        if((XAXA=FORCE(T+CH,Q1,PQ4)))  break;
                        ;
                        DTSTART=DT0+T
                                ;
                        DTFINISH=DTSTART+CH
                    ...
                    while ((DTFINISH-DTK)*DIR<-1.0e-9)
                        ;
                    if((XAXA=FORCE(T+CH,Q1,PQ4)) || (XAXA=FORCE(T+CH,Q1,PQ3)) || (XAXA=FORCE(T+CH,Q1,PQ2)) || (XAXA=FORCE(T+CH,Q1,PQ1))) break;
                    PQ0[0]=DTSTART
                           ;
                    PQ0[1]=DTFINISH
                           ;
        ...
        ;
        if(!((XAXA=FORCE(T+CH,Q1,PQ4)) || (XAXA=FORCE(T+CH,Q1,PQ3)) || (XAXA=FORCE(T+CH,Q1,PQ2)) || (XAXA=FORCE(T+CH,Q1,PQ1))))
        {
            ...
            if  (PZADACHI==2||PZADACHI>3)
                BIX->bix.NH=BX->NH
                            ...
                            /***************ЗАПИСЬ РЕЗУЛЬТАТА***************************************/
                            ;
            BIX->HA=BIX->bix.A*(1+BIX->E)-RZ*(1-ECJ*sqr(sin(I0)*sin(BIX->W)));
            HP=BIX->bix.A*(1-BIX->E)-RZ*(1-ECJ*sqr(sin(I0)*sin(BIX->W)))
               ;
        ...
        if(PZADACHI==3)
        {
            ...
            else if ((xp[2]*sign(DTK-DT0)>0)  && (Z0*sign(DTK-DT0)<0)) /* && (pz!=1)  && (pz!=2))*/
                Nk+=DIR;
            Z0=xp[2];
            if (LONGITUDE>180) LONGITUDE-=360;
            if (LONGITUDE<-180) LONGITUDE+=360;
...
//------------------------------------------------------------------------------
/**
* Calculation of right-hand side of averaged equations.
*
* @param <T> the time count off from initial.
* @param <E1> the vector of nonsingular elements.
* @param <E2> vector of right-hand side of averaged equations value.
*
* @return the right-hand side of averaged equations.
*/
//------------------------------------------------------------------------------
...
int JVC_DLL FORCE(double T,double E1[],double  E2[])
{
    ...
    if  (PATMOSFERA)
    {
        ...
        if  (J0==-1)
        {
            ...
            {
                HODH=1/(0.17527-0.001287*(HP-100))
                     ;
                B2=0.001287
                   ;
            }
