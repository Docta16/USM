...
//
/**
* A example of use of the USM program.
*/
//------------------------------------------------------------------------------
#include "hoko.h"
#include <math.h>
...
//------------------------------------------------------------------------------
/**
* Main program that executes the USM functions.
*
*/
//------------------------------------------------------------------------------
int main (void)
{
    ...
    double epoch_in, inclination_deg, ra_asc_node_deg,
           ...
           semimajor_axis_earth_radii, deg2rad, rad2deg, ballistic_coef;
    int object_in, rev_num_in, I, i, j;
    char input_file[256];
    int NUM_STEPS_TO_PROPAGATE;
    double STEP_SIZE_IN_DAYS;
    int output_type, prmodel, pock, pzadachi;

    rad2deg = 180 / EXAMPLE_PI;
    deg2rad = EXAMPLE_PI / 180;

    /* Read the data from the input.txt file. */
    printf("Do yo want to change the file with the initials parameters?\r\n");
    printf("0 (No) 1 (Yes)\r\n");
    int desccr;
    scanf("%d", &desccr);
    if(desccr == 1)
    {
        printf("Enter the name of the file with the initial parameters:\r\n");
        fflush( stdin );
        gets(input_file);
    }
    else
    ...
    **********************************************************/
    ...
    /* Check for errors. */
    FILE * arch = fopen("output-usm.txt", "w");
    if (L > 0)
    {
        printf("Error return %d from PROGNOZ.\n", L);
    }
    ...
    else if (output_type == XYZ_OUTPUT)
    {
        for (i = 0; i < NUM_STEPS_TO_PROPAGATE; i++)
        {
            ...
            if(arch !=NULL)
            {
                fprintf(arch, "%09.9lf %09.9lf %09.9lf %09.9lf\n",
                        BIX[i].bix.DT,
                        BIX[i].X[0],
                        BIX[i].X[1],
                        BIX[i].X[2]);
    ...
    else if (output_type == KEPLER_MEAN_OUTPUT)
    {
        for (i = 0; i < NUM_STEPS_TO_PROPAGATE; i++)
        {
            ...
            if(arch !=NULL)
            {
                fprintf(arch, "%09.9lf %09.9lf %09.9lf %09.9lf %09.9lf %09.9lf %09.9lf\n",
                        BIX[i].bix.DT,
                        BIX[i].Y[0],
                        BIX[i].Y[1],
                        BIX[i].Y[2],
    ...
    printf("The results has been written in the file output_usm.txt");
