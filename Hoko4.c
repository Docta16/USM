...
//
/**
* A part of the functions that the USM software uses.
*/
//------------------------------------------------------------------------------
#include "hoko.h"
#include <time.h>
...
//void km_x(double E0[],int prz,double EK[],int pech);   /* Û ¬‡ÒË GNSK */
...
//------------------------------------------------------------------------------
/**
* Calculates the result of the prognoz chicl function.
*
* @param <NTOCH> the value of ntoch.
* @param <BX> the value of bx.
* @param <BIX> the value of bix.
* @param <TKNK> the value of tknk.
*
* @return the result of the ejecution of the function.
*/
//------------------------------------------------------------------------------
#ifdef HOKODLL
...
#endif
{
    /*   ‚˚ıÓ‰ PROGNOZ=
          0 -‚ÒÂ ÌÓÏ‡Î¸ÌÓ
          1 -ÌÂÔ‡‚ËÎ¸ÌÓ Á‡‰‡Ì˚ ËÒıÓ‰Ì˚Â ‰‡ÌÌ˚Â
          2 -Ó¯Ë·Í‡ ÓÚÍ˚ÚËˇ Ù‡ÈÎÓ‚
          3 -ÌÂ‰ÓÒÚ‡ÚÓ˜ÌÓ Ô‡ÏˇÚË ‰Îˇ Â¯ÂÌËˇ Á‡‰‡˜Ë
          4 -Ó¯Ë·Í‡ ˜ÚÂÌËˇ ‰‡ÌÌ˚ı ËÁ Ù‡ÈÎÓ‚
          777 -Ò„ÓÂÎ
    */
    ...
    {
        printf("\n Õ≈“ œ¿Ãﬂ“» ƒÀﬂ ƒ»Õ¿Ã»◊≈— O√Œ Ã¿——»¬¿ ¬ œ–Œ÷≈ƒ”–≈ chisint");
        exit(1);
    }
...
#endif
    /******************* ŒÕ≈÷ —◊»“€¬¿Õ»ﬂ ¬’ŒƒÕ€’ ƒ¿ÕÕ€’*************/
    JD=DT0  /*12053-◊»—ÀŒ —”“Œ  Œ“ 31.12.57 ƒŒ 31.12.90*/
       /**************************************************************************/
       ;
    BETTA=L0*L0+H0*H0
          ;
    ...
    MA=fmod(MA,PI2)
       ...
       /****************¬€◊»—À≈Õ»≈ «¬≈«ƒÕŒ√Œ ¬–≈Ã≈Õ»-S0 Õ¿ Õ¿◊¿À‹Õ€… ÃŒÃ≈Õ“*********/
       ;
    S0=ST(JD,DT0-JD)
       /****************************************************************************/
    ...
    km_x(E0,1,EK,0);   /* Û ¬‡ÒË GNSK */
    for(J=0; J<3; J++)
    {
        X[J]=EK[J]/RZ;
        V[J]=EK[J+3]/RZ*86400.0;
    }
    for(J=0; J<6; J++) ER[J]=E0[J];

    /************Õ¿◊¿ÀŒ ÷» À¿ œŒ MŒÃ≈Õ“¿Ã œ–Œ√ÕŒ«¿****************************************************************/
    ...
    do
    {
        /*Ì‡˜‡ÎÓ ˆËÍÎ‡ ÔÓ ÏÓÏÂÌÚ‡Ï ÔÓ„ÌÓÁ‡ ÔË ÔÓ„ÌÓÁÂ Ì‡
        	     ‚ÂÏˇ Ë ÌÓÏÂ ‚ËÚÍ‡ */
        /*% OœPEƒEÀEH»E ÿA√A H,BPEMEH HA◊AÀA » KOH÷A(T » TK)*/
        ...
        if (DTBAKB==0)
        {
            if (PZADACHI==0)
            {
                ...
                if(fabs(TK)>300*PER)
                {
                    printf("\nÕ≈œ–¿¬»À‹ÕŒ Œœ–≈ƒ≈À≈Õ€ ÃŒÃ≈Õ“€ œ–Œ√ÕŒ«»–Œ¬¿Õ»ﬂ(>10 ÒÛÚ)")
                    ;
                    return(1)
                          ;
                }
        ...
        else
        {
            ...
            if(fabs(TK)>100)
            {
                printf("\nÕ≈œ–¿¬»À‹ÕŒ Œœ–≈ƒ≈À≈Õ€ ÃŒÃ≈Õ“€ œ–Œ√ÕŒ«»–Œ¬¿Õ»ﬂ")
                ;
                return(1)
                      ;
            }
        ...
        do     /*ˆËÍÎ ‰Îˇ ÔÂÂÒ˜ÂÚ‡*/
        {
            do    /*ˆËÍÎ ‰Îˇ  ‚˚ıÓ‰‡ Ì‡ ‰‚‡ ‚ËÚÍ‡ Ì‡Á‡‰*/
            {
                do    /*ˆËÍÎ ‰Îˇ  ‚˚ıÓ‰‡ Ì‡ U=0*/
                {
                    ...
                    if(fabs(TK)>1.0e-9)
                        if((XAXA=chisint(X,V,T,T+TK,xL,LL,NV,NI,&NF,&NS,klass,NoR,pr_prod_int))) break;
                    pr_prod_int=1;
                    T+=TK;
                    for(J=0; J<3; J++)
                    {
                        BIX->X[J]=X[J]*RZ;
                        BIX->X[J+3]=V[J]*RZ/86400.0;
                    }
                    x_k(BIX->X,E1,0)
                    ;
                ...
                N_iterac++;
            } /* ÍÓÌÂˆ ˆËÍÎ‡ ÔÓ ÔÓ„ÌÓÁ‡Ï Ì‡ ‰‚‡ ‚ËÚÍ‡ Ì‡Á‡‰*/
            while( N_iterac<3);
            if((XAXA=chisint(X,V,T,T+TK,xL,LL,NV,NI,&NF,&NS,klass,NoR,pr_prod_int))) break;
            if(DTBAKB==1)
        ...
        if(!(XAXA=chisint(X,V,T,T+TK,xL,LL,NV,NI,&NF,&NS,klass,NoR,pr_prod_int)))
        {

        if(DTBAKB==1 && PZADACHI<4)
        {
            DTBAKB=0;
            Nk=BX->NH;
            NT++;
    ...
    /*ÍÓÌÂˆ ˆËÍÎ‡ ÔÓ ÏÓÏÂÌÚ‡Ï ÔÓ„ÌÓÁ‡ ÔË ÔÓ„ÌÓÁÂ Ì‡
    	     ‚ÂÏˇ Ë ÌÓÏÂ ‚ËÚÍ‡ */
...
* @param <tI> the value of tI.
* @param <tF> the value of tF.
* @param <xL> the value of xL.
* @param <LL> the value of LL.
* @param <NV> the value of NV.
* @param <NI> the value of NI.
* @param <NF> the value of NF.
* @param <NS> the value of NS.
* @param <klass> the value of klass.
* @param <NoR> the value of NoR.
* @param <pr> the value of pr.
*
* @return the result of the ejecution of the function.
*/
//------------------------------------------------------------------------------
...
//------------------------------------------------------------------------------
/**
* Calculates the force chis.
*
* @param <x> the value of x.
* @param <V> the value of V.
* @param <tm> the value of tm.
...
//		     else
                ...
                G=alfa+AT1[14]-SZ;      	        	      /* COÀHE◊HO√O */
                SG=sin(G);    	        	        	      /* B«ƒ”T»ﬂ */
                sG=cos(G);    	        	        	      /* [ J0=-1] */
                alfa=sin(delta);       	        	              /* œÀOTHOCT» */
                delta=cos(delta);       	        	              /* C ”◊ETOM */
                sF=(Zp*alfa+delta*(xp*sG+yp*SG))*RR;
                sr=pow((1+sF)*0.5,(AT1[13]+0.006*HR)*0.5);
                sr*=AT1[9]+AT1[10]*HR+AT1[11]*HR*HR+AT1[12]*HR*HR*HR;
                sr+=1.0;
                /* B€◊À œPAB€X ◊ACTE… ƒÀﬂ OœP.  			 */
                /* BO«M”ŸEH»… OT ATMOC‘EP€      			 */
            }
            Ro=ATM(HR,F135,FT,F0,KP,DT0+tm);   	        	/* ¡E« B«ƒ”T»ﬂ */
            Ro*=sr;
            Ro=Ro*QL*RZ*VR;
            F[0]-=Ro*Vx;
            F[1]-=Ro*Vy;
...
*
* @param <E0> the value of eo.
* @param <prz> the value of prz.
* @param <EK> the value of EK.
* @param <pech> the value of pech.
*
*/
//------------------------------------------------------------------------------
void   JVC_DLL km_x(double E0[],int prz,double EK[],int pech)
{
    ...
    if   (pech==1)
    {
        /*
            if   (prz==0)
             pechvekt("\nKm_X.    ÂÔÎÂ › <=> (a,e,i,w,‰‚Û,M):\n_rada",E0,0,5);
        else
             pechvekt("\nKm_X.    ÂÔÎÂ › <=> ( a e i w ‰‚Û u ) :\n_rada",E0,0,5);
